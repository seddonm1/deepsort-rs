...
impl KalmanFilter {
    ...
    */
    ...
    //     let d = Array::from_iter((measurements - mean).t().iter().cloned());

    //     let z = cholesky_factor.solve_triangular(UPLO::Lower, Diag::Unit, &d).unwrap();

    //     let squared_maha = (&z * &z).sum_axis(Axis(0));
